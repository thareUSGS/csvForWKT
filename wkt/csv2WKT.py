...
class Crs2WKT:
    ...
    def __init__(
        self,
        path: str = None,
        filename: str = "naifcodes_radii_m_wAsteroids_IAU2015.csv",
    ):
        """Constructor
        ...
        pathFile = (
            path
            if path is not None
            else os.path.dirname(os.path.realpath(__file__)) + "/../data/"
        )
        self.__df: pd.DataFrame = pd.read_csv(pathFile + filename)
        self.__ellipsoid: pd.DataFrame = None
        self.__datum: pd.DataFrame = None
        self.__planetodetic: pd.DataFrame = None
        self.__projection: pd.DataFrame = None
        self.__wkt: Dict = dict()

    def __merge_planetodetic_data(self) -> pd.DataFrame:
        """Merge ellipsoid, datum and planetodic tables

        Returns:
            pd.DataFrame -- information for creating non projected CRS
        """
        datum = self.__datum.copy()
        ellipsoid = self.__ellipsoid.copy()
        datum["datum"] = (
            datum["authority"].apply(str)
            + ":"
            + datum["version"].apply(str)
            + ":"
            + datum["code"].apply(str)
        )
        datum = datum[
            [
                "name",
        ...
        ellipsoid["ellipsoid"] = (
            ellipsoid["authority"].apply(str)
            + ":"
            + ellipsoid["version"].apply(str)
            + ":"
            + ellipsoid["code"].apply(str)
        )
        ellipsoid = ellipsoid[
            [
                "name",
        ...
        data = pd.merge(self.__planetodetic, datum, on="datum")
        data = pd.merge(data, ellipsoid, on="ellipsoid")

        return data

    def __merge_projection(self, data: pd.DataFrame) -> pd.DataFrame:
        """Merge projection and non projected tables

        Arguments:
            data {pd.DataFrame} -- Non projected table

        Returns:
            pd.DataFrame -- information for creating projected CRS
        """
        dataProjected = data.copy()
        dataProjected["baseCRS"] = (
            dataProjected["authority"].apply(str)
            + ":"
            + dataProjected["version"].apply(str)
            + ":"
            + dataProjected["code"].apply(str)
        )
        dataProjected = dataProjected.rename(
            columns={
                "authority": "authority_baseCrs",
                "version": "version_baseCrs",
                "code": "code_baseCrs",
                "name": "name_baseCrs",
            }
        )
        projection = self.__projection.rename(columns={"id": "projection_id"})
        dataProjected = pd.merge(projection, dataProjected, on="baseCRS")
        return dataProjected

    def __compute_wkts(self):
        """Compute WKTs
        """
        col_names = ["code", "wkt"]
        wkts = pd.DataFrame(columns=col_names)
        planetodeticData = self.__merge_planetodetic_data()
        projectedPlanetodeticData = self.__merge_projection(planetodeticData)
        ...
        for index, wkt in wkts.iterrows():
            try:
                wkt["wkt"].computeWKT()
                self.__wkt[wkt["code"]] = wkt["wkt"].wkt
    ...
    def __process_longitude_positive(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        * Retrograde rotation has longitude positive to East
        ...
        Arguments:
            df {pd.DataFrame} -- pandas containing the IAU CSV file

        Returns:
            pd.DataFrame -- IAU information with updated longitude sens
        """
    ...
    def __process_zero_longitude(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        Returns:
            pd.DataFrame -- IAU information with updated longitude value and position
        """
        df.loc[df["origin_lon_pos"].isnull(), "origin_lon_pos"] = "0.0"
        df.loc[
            df["origin_long_name"].isnull(), "origin_long_name"
        ] = "Reference_Meridian"
        return df

    def __compute_ellipsoid(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        * remark
        ...
        .. code-block::
            ...
            -------------
                                             ...
                                             /
             bodies shape  ----- >  sphere  /  OR (code: 00)
                           |                 \
        ...
        Returns:
            pd.DataFrame -- a new table with IAU information with the following reference ellipsoid : sphere, ellipse, triaxial
        """
        ...
        ellipsoid = ellipsoid.rename(
            columns={
                "Naif_id": "code",
                "Body": "name",
                "IAU2015_Semimajor": "semiMajorAxis",
                "IAU2015_Axisb": "semiMedianAxis",
                "IAU2015_Semiminor": "semiMinorAxis",
            }
        )
        ...
        ellipsoid.loc[0::3, "name"] = (
            ellipsoid.loc[0::3, "name"].str[:]
            + " ("
            + ellipsoid.loc[0::3, "version"].apply(str)
            + ") - Sphere"
        )
        ...
        ellipsoid.loc[1::3, "name"] = (
            ellipsoid.loc[1::3, "name"].str[:]
            + " ("
            + ellipsoid.loc[1::3, "version"].apply(str)
            + ")"
        )
        ellipsoid.loc[1::3, "inverseFlatenning"] = ellipsoid.loc[
            1::3, "semiMajorAxis"
        ] / (
            ellipsoid.loc[1::3, "semiMajorAxis"] - ellipsoid.loc[1::3, "semiMinorAxis"]
        )
        ellipsoid["inverseFlatenning"] = ellipsoid["inverseFlatenning"].replace(
            np.inf, 0
        )

        ellipsoid.loc[2::3, "type"] = "TRIAXIAL"
        ellipsoid.loc[2::3, "code"] = ellipsoid.loc[2::3, "code"] * 100 + 2
        ellipsoid.loc[2::3, "name"] = (
            ellipsoid.loc[2::3, "name"].str[:]
            + " ("
            + ellipsoid.loc[2::3, "version"].apply(str)
            + ")"
        )

        # Exceptions to the general cases
        # case1 =  > Use R_m = (a+b+c)/3 as mean radius when mean radius is not defined
        ellipseBodyToRemove = ellipsoid.query(
            "IAU2015_Mean == -1 and (type != 'TRIAXIAL' and type != 'SPHERE')"
        )
        ellipsoid = ellipsoid.drop(ellipseBodyToRemove.index)
        medianSphereToSet = ellipsoid.query("IAU2015_Mean == -1 and type == 'SPHERE'")
        ellipsoid.loc[medianSphereToSet.index, "IAU2015_Mean"] = (
            ellipsoid.loc[medianSphereToSet.index, "semiMajorAxis"]
            + ellipsoid.loc[medianSphereToSet.index, "semiMinorAxis"]
            + ellipsoid.loc[medianSphereToSet.index, "semiMedianAxis"]
        ) / 3
        ellipsoid.loc[
            medianSphereToSet.index, "remark"
        ] += "Use R_m = (a+b+c)/3 as mean radius. "

        # Spherical case
        # set mean radius for triaxial bodies
        triaxialBodies = ellipsoid.query(
            "semiMajorAxis != semiMedianAxis and semiMinorAxis != semiMedianAxis and type == 'SPHERE'"
        )
        ellipsoid.loc[triaxialBodies.index, "semiMajorAxis"] = ellipsoid.loc[
            triaxialBodies.index, "IAU2015_Mean"
        ]
        ellipsoid.loc[triaxialBodies.index, "semiMinorAxis"] = ellipsoid.loc[
            triaxialBodies.index, "IAU2015_Mean"
        ]
        ellipsoid.loc[triaxialBodies.index, "semiMedianAxis"] = ""
        ellipsoid.loc[
            triaxialBodies.index, "remark"
        ...
        noTriaxialBodies_index = pd.Index(
            list(
                set(sphere.index.tolist()).difference(
                    set(triaxialBodies.index.tolist())
                )
            )
        )
        ellipsoid.loc[noTriaxialBodies_index, "semiMinorAxis"] = ellipsoid.loc[
            noTriaxialBodies_index, "semiMajorAxis"
        ]
        ellipsoid.loc[noTriaxialBodies_index, "semiMedianAxis"] = ""
        ellipsoid.loc[
            noTriaxialBodies_index, "remark"
        ] = "Use semi-major radius as sphere for interoperability. "

        # Ellipse case
        triaxialBodies = ellipsoid.query(
            "semiMajorAxis != semiMedianAxis and semiMinorAxis != semiMedianAxis and type == 'ELLIPSE'"
        )
        ellipse = ellipsoid.query("type == 'ELLIPSE'")
        noTriaxialBodies_index = pd.Index(
            list(
                set(ellipse.index.tolist()).difference(
                    set(triaxialBodies.index.tolist())
                )
            )
        )
        ...
        # Triaxial case
        triaxialBodies = ellipsoid.query(
            "semiMajorAxis != semiMedianAxis and semiMinorAxis != semiMedianAxis and type == 'TRIAXIAL'"
        )
        triaxial = ellipsoid.query("type == 'TRIAXIAL'")
        noTriaxialBodies_index = pd.Index(
            list(
                set(triaxial.index.tolist()).difference(
                    set(triaxialBodies.index.tolist())
                )
            )
        )
        ...
        remarkNoSphericalApprox = ellipsoid.query(
            "type == 'ELLIPSE' and semiMajorAxis == semiMinorAxis"
        )
        ...
        ellipsoid[
            "remark"
        ] += "Source of IAU Coordinate systems: doi://10.1007/s10569-017-9805-5"
    ...
    def __compute_datum(
        self, df: pd.DataFrame, ellipsoid: pd.DataFrame
    ) -> pd.DataFrame:
        """Compute the datum
        ...
        * primeMeridianValue
        ...
        Arguments:
            df {pd.DataFrame} -- IAU information
            ellipsoid {pd.DataFrame} -- ellipsoid information

        Returns:
            pd.DataFrame -- datum information
        """
        ...
        df_datum = df_datum.rename(
            columns={
                "Body": "body",
                "origin_long_name": "primeMeridianName",
                "origin_lon_pos": "primeMeridianValue",
            }
        )
    ...
    def __split_datum_case(self, df: pd.DataFrame):
        """From datum information, split the information into two categories : ographic, ocentric

        Arguments:
            df {pd.DataFrame} -- datum information

        Returns:
            (pd.DataFrame, pd.DataFrame) -- (ographic, ocentric)
        """
        ...
        ographic = df[df.longitudeDirection.notnull()].query(
            "Body != 'Sun' and Body != 'Moon'"
        )
        return (ographic, ocentric)

    def __compute_planetodetic(
        self, df: pd.DataFrame, datum: pd.DataFrame
    ) -> pd.DataFrame:
        ...
        * longitudeDirection
        ...
        Arguments:
            df {pd.DataFrame} -- IAU information
            datum {pd.DataFrame} -- datum information

        Returns:
            pd.DataFrame -- planetodetic information
        """
        df = df.rename(columns={"rotation": "longitudeDirection"})
        df_planeto = pd.merge(
            datum[
                ["Naif_id", "authority", "version", "code", "name", "ellipsoid", "type"]
            ],
            df[["Naif_id", "longitudeDirection", "Body"]],
            on="Naif_id",
        )

        ographic, ocentric = self.__split_datum_case(df_planeto)

        historicalReasons = ocentric.query(
            "(Body == 'Sun' or Body == 'Moon') and type != 'SPHERE'"
        )
        ...
        ocentric.loc[ellipse.index, "code"] = (
            ocentric.loc[ellipse.index, "Naif_id"] * 100 + 2
        )
        ocentric.loc[ellipse.index, "id"] = 2

        triaxial = ocentric.query("type == 'TRIAXIAL'")
        ocentric.loc[triaxial.index, "code"] = (
            ocentric.loc[triaxial.index, "Naif_id"] * 100 + 4
        )
        ...
        ographic.loc[ellipse.index, "code"] = (
            ocentric.loc[ellipse.index, "Naif_id"] * 100 + 1
        )
        ographic.loc[ellipse.index, "id"] = 1

        triaxial = ographic.query("type == 'TRIAXIAL'")
        ographic.loc[triaxial.index, "code"] = (
            ographic.loc[triaxial.index, "Naif_id"] * 100 + 3
        )
    ...
    def __compute_projection(self, planetodetic: pd.DataFrame) -> pd.DataFrame:
        """
        Information:
        * authority
        * version
        ...
        * parameterValue
        ...
        planetodeticCopy["baseCRS"] = (
            planetodeticCopy["authority"].apply(str)
            + ":"
            + planetodeticCopy["version"].apply(str)
            + ":"
            + planetodeticCopy["code"].apply(str)
        )
        projection_df = pd.merge(
            planetodeticCopy[["id", "baseCRS", "code"]], df, on="id", how="left"
        )
        projection_df["code"] = (
            projection_df["code"] - projection_df["id"] + projection_df["proj_id"]
        )
        projection_df = projection_df.sort_values(by="code")
        projection_df = projection_df.astype({"code": int})
        return projection_df

    def process(self):
        """Processes the CSV file to create WKTs.
        """
        df = self.__df.copy()
        ...
        self.__ellipsoid = self.__compute_ellipsoid(df)
        self.__datum = self.__compute_datum(df, self.__ellipsoid)
        self.__planetodetic = self.__compute_planetodetic(df, self.__datum)
        self.__projection = self.__compute_projection(self.__planetodetic)
        self.__compute_wkts()

    def getWkts(self) -> Dict:
        """Retrieves the WKTs.
    ...
    def save(self):
        """Save the WKTs as CSV files and print the WKTs on stdout.
        """
        ...
        planetodetic = self.__planetodetic[
            [
                "authority",
                "version",
                "code",
